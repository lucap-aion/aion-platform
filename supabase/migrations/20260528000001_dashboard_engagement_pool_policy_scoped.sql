-- ============================================================
-- Align Dashboard engagement pool with Insights deep-dive.
--
-- Two divergences existed between the AdminDashboard Engagement
-- tiles and the AdminInsights "Client deep dive" funnel for the
-- same brand/shop/date scope:
--
--   1. Denominator. The RPC's `pool_*` queries counted every
--      profile with `role='customer' AND brand_id = ANY(...)`,
--      whether or not the customer had a live policy in scope.
--      Insights instead derives `uniqueCustomers` from policies
--      filtered to `status='live'` plus the active shop/date
--      filters. The pool therefore drifted above the deep-dive
--      denominator (e.g. 242 vs 237 for All-Brands/All-time).
--
--   2. "Filled" semantics. `pool_profilation_started` and
--      `pool_profiled` treated any non-null value as filled, so
--      empty strings ('') counted as a populated profile field.
--      Insights uses JS truthiness, which rejects ''. This made
--      Dashboard's Profilation Completed count higher than the
--      deep-dive (e.g. 17 vs 15).
--
-- This migration:
--   * Requires every `pool_*` query to EXISTS a live policy in
--     scope (brand_id, shop_id, start_date all matched against
--     the RPC parameters), removing the policy-less profile
--     inflation. The previous `pr.created_at` date filter is
--     dropped — date scope now flows through the policy.
--   * Switches the optional-field check to
--     `NULLIF(field, '') IS NOT NULL` so empty strings no longer
--     count as filled, matching the Insights truthiness check.
--
-- Legacy `customers_registered`, `customers_profiled` and
-- `customers_with_feedback` already operate over the policy set
-- and stay unchanged here (no rate consumer divides by them).
-- ============================================================
  ...
  SELECT json_build_object(
    ...
    'customers_count',       (SELECT count(DISTINCT pol.customer_id)::bigint
                                ...
                                WHERE pol.brand_id = ANY(v_brand_ids)
                                  ...
                                  AND (p_shop_ids     IS NULL OR pol.shop_id      = ANY(p_shop_ids))),
    -- ── Engagement pool ────────────────────────────────────────
    -- Every pool_* query requires a live policy for the customer
    -- inside the active brand/shop/date scope, matching the
    -- Insights deep-dive denominator (`uniqueCustomers`).
                                ...
                                WHERE pr.role = 'customer'
                                  ...
                                  AND EXISTS (
                                    ...
                                    WHERE pol2.customer_id = pr.id
                                      ...
                                      AND (p_shop_ids  IS NULL OR pol2.shop_id    = ANY(p_shop_ids))
    ...
    -- Registered AND at least one of the 8 optional profile
    -- fields is populated. Text fields use NULLIF so empty
    -- strings do not count. date_of_birth is timestamptz and
    -- cannot hold an empty string, so IS NOT NULL is enough.
    'pool_profilation_started', (SELECT count(*)::bigint
                                ...
                                WHERE pr.role = 'customer'
                                  ...
                                  AND (
                                    ...
                                    OR NULLIF(pr.country,       '') IS NOT NULL
                                    OR NULLIF(pr.city,          '') IS NOT NULL
                                    OR NULLIF(pr.postcode,      '') IS NOT NULL
                                    OR NULLIF(pr.address,       '') IS NOT NULL
                                    OR NULLIF(pr.province,      '') IS NOT NULL
                                    OR NULLIF(pr.nationality,   '') IS NOT NULL
                                    OR NULLIF(pr.phone_number,  '') IS NOT NULL
                                  )
                                  AND (p_customer_ids IS NULL OR pr.id = ANY(p_customer_ids))
                                  AND EXISTS (
                                    SELECT 1 FROM public.policies pol2
    ...
    -- Registered AND all 8 optional fields populated. Text
    -- fields use NULLIF so empty strings do not count;
    -- date_of_birth is timestamptz and uses IS NOT NULL only.
    'pool_profiled',         (SELECT count(*)::bigint
                                ...
                                WHERE pr.role = 'customer'
                                  ...
                                  AND NULLIF(pr.country,       '')       IS NOT NULL
                                  AND NULLIF(pr.city,          '')       IS NOT NULL
                                  AND NULLIF(pr.postcode,      '')       IS NOT NULL
                                  AND NULLIF(pr.address,       '')       IS NOT NULL
                                  AND NULLIF(pr.province,      '')       IS NOT NULL
                                  AND NULLIF(pr.nationality,   '')       IS NOT NULL
                                  AND NULLIF(pr.phone_number,  '')       IS NOT NULL
                                  AND (p_customer_ids IS NULL OR pr.id = ANY(p_customer_ids))
                                  AND EXISTS (
                                    SELECT 1 FROM public.policies pol2
                                    WHERE pol2.customer_id = pr.id
