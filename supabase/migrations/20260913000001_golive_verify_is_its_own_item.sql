-- ==============================|| VERIFYING IS NOT A FIELD ||============================== --
-- "Create the brand record" listed five things: description, website, customer-care address,
-- registered office — and "the status is verified". The first four are data the onboarding
-- run fills in on its own. The fifth is a DECISION, and a consequential one: the portal's
-- brand picker lists every verified brand to anonymous visitors, and the admin dashboards,
-- reports, covers and claims all scope to them.
--
-- So a brand whose record was completely filled in sat unticked, blocking, under a sentence
-- whose every checkable clause was true. The status moves to its own key.
--
-- Ships with the client bundle that adds the matching item: an extra key nothing reads is
-- harmless, a missing one would make the new item permanently manual.
