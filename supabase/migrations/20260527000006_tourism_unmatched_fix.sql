-- Fix: ambiguous "policy_count" / "city" in tourism_unmatched_shops.
-- Same root cause as 20260527000005: RETURNS TABLE column names become
-- OUT params, colliding with the unqualified order-by column.
-- Use #variable_conflict use_column directive so future refs prefer columns,
-- and rename the inner aliases just to be safe.
...
begin
  ...
  return query
    ...
    from (
      select
        coalesce(s.city, '(null)') as cte_city,
