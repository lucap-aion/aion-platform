...
-- "Obtain the exact category list" said, when empty, "nothing has arrived through the sales
-- integration yet". When it was NOT empty it counted rows that had never been near the sales
-- integration: `catalogues` is filled both by real sales and by the demo generator reading the
-- house's own website, and only the first kind is the vocabulary a brand will actually send.
--
-- Ferragamo read "5 categories, none of them costed" with zero sales on file. Roberto Coin,
-- which has 517 rows from real sales, reads the same shape and means something entirely
-- different by it. The two are distinguishable: the ingestion writes no `price_source`, the
-- storefront read writes 'storefront'.
--
-- So the item is satisfied by REAL sales categories, and a house that only has the website's
-- guess is told exactly that, rather than being asked to cost a list that is not yet the one
-- it will send.
