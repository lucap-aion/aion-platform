-- ==============================|| A CATALOGUE WITH NO PRICES IN IT ||============================== --
-- generate_brand_demo_data guarded on how many rows a brand's catalogue had, and then built
-- from the rows that carry a PRICE. Those are not the same number.
--
-- It never mattered while every catalogue arrived priced. It matters now: a house behind a
-- bot challenge has its catalogue recovered from its own sitemap — names, categories, item
-- codes, packshots, and no prices, because a price is only ever on the page. Damiani passed
-- the check with 676 pieces, inserted none of them into `catalogues`, and the policy loop
-- then selected an item that did not exist and wrote its NULL id into policies.item_id:
--
--   null value in column "item_id" of relation "policies" violates not-null constraint
--
-- Counting priced rows sends it down the path that already exists for exactly this — ask for
-- a typical retail ticket and spread demo prices around it. And the loop now checks it
-- actually found a piece, because a cover with nothing behind it is not a cover.

CREATE OR REPLACE FUNCTION public.generate_brand_demo_data(p_brand_id bigint, p_customers integer DEFAULT 40, p_policies integer DEFAULT 60, p_avg_ticket numeric DEFAULT NULL::numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  -- PRICED rows, not rows. This counted every row in the catalogue, and the insert below
  -- takes only the ones with a price — so a house whose catalogue was recovered without
  -- prices (one behind a bot challenge, read from its own sitemap) satisfied the check with
  -- 676 pieces, inserted none of them, and then dereferenced an item it had never found.
  -- A price-less catalogue is exactly the case the knowledge fallback already handles by
  -- asking for a typical ticket; it just was not being reached.
  select count(*) into v_products
    from public.storefront_products
   where brand_id = p_brand_id and price is not null;
  ...
  for i in 1..p_policies loop
    ...
    begin
      ...
      -- Belt as well as braces: a cover with no piece behind it is not a cover, and
      -- inserting one fails on a NOT NULL half a loop later with nothing said about why.
