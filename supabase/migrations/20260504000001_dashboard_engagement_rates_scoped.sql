-- ============================================================
-- Engagement rate denominators/numerators must share the same scope.
--
-- `customers_count` (the denominator for every engagement rate card) is
-- distinct customer_id of LIVE policies in the brand/shop/period filter.
-- The previous client computed `profiledCount` and `feedbackUserCount`
-- from `profiles` / `feedback` directly with only a brand_id filter, so
-- when a period was selected the numerator stayed all-time while the
-- denominator shrank — yielding ratios above 100% (e.g. 340%, 120%).
--
-- Move both numerators into admin_dashboard_aggregates so they reuse
-- the same policy-scoped customer set as customers_count.
-- ============================================================
