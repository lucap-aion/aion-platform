-- ============================================================
-- Profilation: ignore business info synced from eplay.
--
-- Some profiles are B2B/eplay records (Italian SRLs etc.) loaded
-- with role='customer'. They carry business info (vat, entity_name,
-- business_address) and eplay writes the COMPANY address into the
-- consumer `address` column. That made these records show partial
-- profilation (e.g. 11%) purely from business-sourced data.
--
-- Fix (matches the client AdminCustomers/AdminInsights logic):
--   1. A text profile field counts as "filled" only when it is
--      NOT NULL *and* not an empty string. Previously empty strings
--      ('') counted as filled.
--   2. On a profile that carries business info, the `address` field
--      holds the company address and is NOT counted toward profilation.
--   vat/entity_name/business_address are never profilation fields
--   themselves, so they keep being ignored.
--
-- Only the started/completed (profiled) definitions change; pool_total,
-- pool_registered, feedback, claims and policy_stats are untouched.
-- ============================================================
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- (empty strings excluded; `address` ignored on business records).
    'pool_profilation_started', (SELECT count(*)::bigint
                                ...
                                WHERE pr.role = 'customer'
                                  ...
                                  AND (
                                    ...
                                    OR (pr.country      IS NOT NULL AND pr.country      <> '')
                                    OR (pr.city         IS NOT NULL AND pr.city         <> '')
                                    OR (pr.postcode     IS NOT NULL AND pr.postcode     <> '')
                                    OR (pr.address      IS NOT NULL AND pr.address      <> '' AND NOT (
                                          (pr.vat              IS NOT NULL AND pr.vat              <> '') OR
                                          (pr.entity_name      IS NOT NULL AND pr.entity_name      <> '') OR
                                          (pr.business_address IS NOT NULL AND pr.business_address <> '')))
                                    OR (pr.province     IS NOT NULL AND pr.province     <> '')
                                    OR (pr.nationality  IS NOT NULL AND pr.nationality  <> '')
                                    OR (pr.phone_number IS NOT NULL AND pr.phone_number <> '')
                                  )
                                  AND (p_customer_ids IS NULL OR pr.id = ANY(p_customer_ids))
                                  AND (p_customer_ids IS NOT NULL OR p_from_date IS NULL OR pr.created_at >= p_from_date)
                                  AND (p_customer_ids IS NOT NULL OR p_to_date   IS NULL OR pr.created_at <= p_to_date)
    ...
    -- All 8 optional fields populated (empty strings excluded;
    -- `address` ignored on business records, so a B2B record can never count).
    'pool_profiled',         (SELECT count(*)::bigint
                                ...
                                WHERE pr.role = 'customer'
                                  ...
                                  AND pr.country      IS NOT NULL AND pr.country      <> ''
                                  AND pr.city         IS NOT NULL AND pr.city         <> ''
                                  AND pr.postcode     IS NOT NULL AND pr.postcode     <> ''
                                  AND pr.address      IS NOT NULL AND pr.address      <> '' AND NOT (
                                          (pr.vat              IS NOT NULL AND pr.vat              <> '') OR
                                          (pr.entity_name      IS NOT NULL AND pr.entity_name      <> '') OR
                                          (pr.business_address IS NOT NULL AND pr.business_address <> ''))
                                  AND pr.province     IS NOT NULL AND pr.province     <> ''
                                  AND pr.nationality  IS NOT NULL AND pr.nationality  <> ''
                                  AND pr.phone_number IS NOT NULL AND pr.phone_number <> ''
                                  AND (p_customer_ids IS NULL OR pr.id = ANY(p_customer_ids))
                                  AND (p_customer_ids IS NOT NULL OR p_from_date IS NULL OR pr.created_at >= p_from_date)
                                  AND (p_customer_ids IS NOT NULL OR p_to_date   IS NULL OR pr.created_at <= p_to_date)
                                  AND (p_shop_ids IS NULL OR EXISTS (
