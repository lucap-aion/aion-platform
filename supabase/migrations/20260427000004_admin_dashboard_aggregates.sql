-- ============================================================
-- Admin dashboard: single SECURITY DEFINER RPC returning every
-- metric the home page needs in one round trip. Bypasses RLS
-- (admin-only authZ check inside) to skip the heavy planning
-- cost of OR'd RLS policies on brands/shops/profiles/claims/
-- policies. Returns a single JSON scalar so PostgREST doesn't
-- wrap it in an array.
-- ============================================================
...
DECLARE
  ...
  v_result    json;
BEGIN
  -- AuthZ: only platform admins may call this aggregate.
  -- IS DISTINCT FROM handles the NULL case (anon / unauthenticated).
  IF public.get_my_role() IS DISTINCT FROM 'admin' THEN
    RAISE EXCEPTION 'Forbidden' USING ERRCODE = '42501';
  END IF;

  -- Default brand scope = all verified brands.
    ...
    'policy_stats',     (
      ...
      FROM (
        SELECT
          ...
          coalesce(sum(coalesce(p.cogs, 0)                       * coalesce(p.quantity, 1)), 0)::numeric AS total_cogs,
          coalesce(sum(coalesce(p.recommended_retail_price, 0)   * coalesce(p.quantity, 1)), 0)::numeric AS total_rrp,
          coalesce(sum(coalesce(p.selling_price, 0)              * coalesce(p.quantity, 1)), 0)::numeric AS total_selling_price,
