-- ============================================================
-- Dashboard financials: report BOTH the full retail price (total_rrp) and the
-- retail value actually covered by the program (total_covered_value =
-- policies.covered_value, frozen at sale = LEAST(rrp, brand max_covered_value)).
-- The activation fee is computed on total_covered_value; total_rrp stays the
-- raw retail so it is comparable with total_selling_price.
-- dashboard_policy_stats changes its return type, so it is dropped and recreated.
-- ============================================================


CREATE OR REPLACE FUNCTION public.admin_dashboard_aggregates(p_brand_ids integer[] DEFAULT NULL::integer[], p_from_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_to_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_customer_ids uuid[] DEFAULT NULL::uuid[], p_shop_ids integer[] DEFAULT NULL::integer[])
 RETURNS json
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT json_build_object(
                                ...
                                WHERE pr.role = 'customer'
                                  ...
                                  AND EXISTS (
                                    ...
                                    WHERE fb.user_id = pr.id
                                      ...
                                      AND fb.impersonated_by IS NULL
                                  )),
    ...
    coalesce(sum(coalesce(p.covered_value, p.recommended_retail_price, 0)), 0)::numeric AS total_covered_value,
...
DROP FUNCTION IF EXISTS public.dashboard_policy_stats(int[], timestamptz, timestamptz, uuid[], int[]);
CREATE OR REPLACE FUNCTION public.dashboard_policy_stats(p_brand_ids integer[] DEFAULT NULL::integer[], p_from_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_to_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_customer_ids uuid[] DEFAULT NULL::uuid[], p_shop_ids integer[] DEFAULT NULL::integer[])
 RETURNS TABLE(brand_id integer, covers bigint, total_cogs numeric, total_rrp numeric, total_covered_value numeric, total_selling_price numeric, latest_start_date timestamp with time zone)
 LANGUAGE sql
 STABLE
AS $function$
  SELECT
...
GRANT EXECUTE ON FUNCTION public.dashboard_policy_stats(int[], timestamptz, timestamptz, uuid[], int[]) TO anon, authenticated, service_role;
