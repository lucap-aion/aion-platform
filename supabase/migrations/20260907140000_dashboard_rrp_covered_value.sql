-- ============================================================
-- Dashboard financials: the activation-fee base (total_rrp) is the retail value
-- actually covered by the program (policies.covered_value, frozen at sale =
-- LEAST(rrp, brand max_covered_value)), not the full retail price.
-- E.g. a EUR 480k necklace counts EUR 100k. Falls back to the raw RRP when
-- covered_value is NULL. Bodies below are the live definitions with only the
-- total_rrp expression changed.
-- ============================================================
    ...
    'policy_stats',          (
      ...
      FROM (
        SELECT
          ...
          coalesce(sum(coalesce(p.covered_value, p.recommended_retail_price, 0)), 0)::numeric AS total_rrp,
...
CREATE OR REPLACE FUNCTION public.dashboard_policy_stats(p_brand_ids integer[] DEFAULT NULL::integer[], p_from_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_to_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_customer_ids uuid[] DEFAULT NULL::uuid[], p_shop_ids integer[] DEFAULT NULL::integer[])
 RETURNS TABLE(brand_id integer, covers bigint, total_cogs numeric, total_rrp numeric, total_selling_price numeric, latest_start_date timestamp with time zone)
 LANGUAGE sql
 STABLE
AS $function$
  SELECT
