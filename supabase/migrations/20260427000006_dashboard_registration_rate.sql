-- ============================================================
-- Add customers_registered (profiles where registered_at IS NOT NULL)
-- to admin_dashboard_aggregates so the home page can show a
-- registration-rate metric. customers_count stays as-is.
-- ============================================================
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'customers_registered',  (SELECT count(*)::bigint FROM public.profiles p WHERE p.role = 'customer' AND p.brand_id = ANY(v_brand_ids) AND p.registered_at IS NOT NULL),
