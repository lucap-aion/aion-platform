...
const AdminCustomerDetail = () => {
  ...
  const { data: customer, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select("id, first_name, last_name, email, phone_number, address, city, province, country, postcode, nationality, date_of_birth, avatar, created_at, registered_at, brand_id, brands(name, logo_small)")
        .eq("id", customerId!)
        .single();
  ...
  const { data: covers } = useQuery({
    queryKey: ["admin-customer-covers", customerId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("policies")
        .select("id, start_date, expiration_date, status, selling_price, recommended_retail_price, catalogues(name, picture), brands(name, logo_small), shops(name)")
  ...
  const { data: claims } = useQuery({
    queryKey: ["admin-customer-claims", customerId],
    queryFn: async () => {
      const policyIds = (covers || []).map((c) => c.id);
      if (!policyIds.length) return [];
      const { data, error } = await supabase
        .from("claims")
        .select("id, type, status, created_at, policies!claims_policy_id_fkey(catalogues(name))")
  ...
  const brand = (customer as any).brands as { name: string | null; logo_small: string | null } | null;
            ...
            <p className="mt-0.5 text-sm text-muted-foreground flex items-center gap-2">
              {brand?.name && (
                <span className="inline-flex items-center gap-1.5">
                  {brand.logo_small && <img src={brand.logo_small} alt="" className="h-4 w-4 object-contain" />}
                  {brand.name} ·
                </span>
              )}
              ...
              {!covers?.length ? (
                ...
                <div className="divide-y divide-border">
                  {covers.map((cover) => {
                    ...
                    const coverBrand = (cover as any).brands as { name: string | null; logo_small: string | null } | null;
                    return (
                      <div
                        key={cover.id}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">
                            ...
                            {coverBrand?.name && ` · ${coverBrand.name}`}
                            {(cover as any).shops?.name && ` · ${(cover as any).shops.name}`}
                          </p>
                        </div>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-foreground">
                      {[customer.address, customer.city, customer.province, customer.postcode, customer.country].filter(Boolean).join(", ") || "—"}
                    </p>
                  </div>
                </div>
                {customer.nationality && (
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-foreground">{customer.nationality}</p>
                    </div>
                  </div>
                )}
                {customer.date_of_birth && (
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-foreground">{fmt(customer.date_of_birth)}</p>
                    </div>
                  </div>
                )}
              </div>
